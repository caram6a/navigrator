...
export default function MBTITestPage() {
  ...
  const handleSaveGameSession = () => {
    ...
    const sessionData = { date: new Date().toISOString(), noGames, games: selectedGames };
    ...
    Object.entries(selectedGames).forEach(([gameId, data]: [string, any]) => {
      if (data.rating) { ratings[gameId] = ratings[gameId] || {}; ratings[gameId][userId] = data.rating; }
    });
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key >= "1" && e.key <= "7") { e.preventDefault(); handleAnswer(parseInt(e.key)); }
    else if (e.key === "Enter") { e.preventDefault(); if (currentQuestion < QUESTIONS.length - 1) nextQuestion(); else if (Object.keys(answers).length === 32) handleSubmit(); }
    ...
    else if (e.key === "Backspace") { e.preventDefault(); setAnswers(prev => { const n = { ...prev }; delete n[currentQuestion + 1]; return n; }); }
  };
  ...
  const renderDimension = (key: string, dim: DimensionData) => {
    ...
    const scaleNames: Record<string, string> = { EI: "Экстраверсия — Интроверсия", SN: "Интуиция — Сенсорика", TF: "Мышление — Чувство", JP: "Суждение — Восприятие" };
    return (
            ...
            {noGames ? (
              ...
              <div className="space-y-4">
                {GAMES.map(game => (
                  <div key={game.id} className="p-3 rounded-lg border">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" checked={!!selectedGames[game.id]}
                        onChange={(e) => { if (e.target.checked) setSelectedGames(prev => ({ ...prev, [game.id]: { times: 1, helper: "", rating: 0 } })); else { const ns = { ...selectedGames }; delete ns[game.id]; setSelectedGames(ns); } }}
                        className="w-4 h-4" />
                    ...
                    {selectedGames[game.id] && (
                      <div className="mt-3 ml-7 space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <input type="number" min={1} value={selectedGames[game.id].times}
                            onChange={(e) => setSelectedGames(prev => ({ ...prev, [game.id]: { ...prev[game.id], times: parseInt(e.target.value) || 1 } }))}
                            className="w-20 px-2 py-1 rounded border bg-background text-sm" />
                        </div>
                        <div className="flex items-center gap-2">
                          <label className="text-sm w-32">Наставник:</label>
                          <select value={selectedGames[game.id].helper}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <input type="number" min={1} max={10} value={selectedGames[game.id].rating || ""}
                            onChange={(e) => setSelectedGames(prev => ({ ...prev, [game.id]: { ...prev[game.id], rating: parseInt(e.target.value) || 0 } }))}
                            className="w-20 px-2 py-1 rounded border bg-background text-sm text-center" placeholder="1-10" />
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <Button onClick={() => { handleSaveGameSession(); router.push("/profile"); }}>Сохранить и перейти в профиль</Button>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-2xl mx-auto space-y-8">
          ...
          <div className="p-6 rounded-xl border bg-card text-center">
            <h2 className="text-2xl font-bold text-purple-600 dark:text-purple-400 mb-2">{result.description.title}</h2>
          ...
          <div className="p-6 rounded-xl border bg-card">
            ...
            <ul className="space-y-2">{result.description.strengths.map((s: string, i: number) => (<li key={i} className="flex items-center gap-2 text-sm"><CheckCircle className="h-4 w-4 text-green-500 flex-shrink-0" />{s}</li>))}</ul>
          </div>
          <div className="p-6 rounded-xl border bg-card">
            <h3 className="font-semibold text-lg mb-3">Зоны роста</h3>
            <ul className="space-y-2">{result.description.growth.map((g: string, i: number) => (<li key={i} className="flex items-center gap-2 text-sm"><Brain className="h-4 w-4 text-purple-500 flex-shrink-0" />{g}</li>))}</ul>
          </div>
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold mb-2"><Brain className="h-8 w-8 inline mr-2 text-purple-500" />MBTI-тест</h1>
          {isRetake && <p className="text-sm text-amber-600 dark:text-amber-400">Повторное прохождение</p>}
        </div>
        <div ref={divRef} tabIndex={-1} style={{ outline: "none" }} onKeyDown={handleKeyDown}>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-2 justify-center">
                {[1, 2, 3, 4, 5, 6, 7].map((value) => (
                  <button key={value} type="button" onClick={() => { handleAnswer(value); divRef.current?.focus(); }}
                    className={"w-10 h-10 rounded-full text-sm font-medium transition-all " + (currentAnswer === value ? "bg-primary text-primary-foreground scale-110" : "bg-muted hover:bg-muted/80 text-muted-foreground")}>
                    {value}
                  </button>
                ))}
              </div>
          ...
          <div className="flex justify-between mt-8">
            <Button variant="outline" onClick={prevQuestion} disabled={currentQuestion === 0}><ArrowLeft className="h-4 w-4 mr-2" /> Назад</Button>
            {currentQuestion < QUESTIONS.length - 1 ? (
              <Button onClick={nextQuestion} disabled={!currentAnswer}>Далее</Button>
            ) : (
              <Button onClick={handleSubmit} disabled={submitting || answeredCount !== 32}>{submitting ? "Отправка..." : "Завершить тест"}</Button>
            )}
          </div>
          <p className="text-xs text-center text-muted-foreground mt-4">1-7 — выбор, Enter — далее, ← → — навигация</p>
        </div>
      </div>
    </div>
  );
